...
public class Simplex {
    ...
    public static void executarSimplex(String outputFile) {
        ...
        String[] variaveisCalculo = setCabecalho(nColunas);
        ...
        while (existemNumerosNegativos(matrizSimplex[nLinhas - 1])) {
            ...
            Writer.imprimirIteração(resultados, matrizSimplex, iteracao, outputFile,variaveisCalculo);
        ...
        imprimirIteração(resultados, matrizSimplex, iteracao, outputFile, variaveisCalculo);
//        String[] res = resultados;
    ...
    private static String[] setCabecalho(int nColunas) {
        String[] variaveisCalculo = new String[nColunas];
        if (InputDataProcessing.MAXIMIZACAO) {
            for (int i = 0; i < variaveis.length; i++) {
                variaveisCalculo[i] = variaveis[i];
            }
        } else {
            for (int i = variaveis.length; i < variaveisCalculo.length; i++) {
                variaveisCalculo[i] = resultados[i - variaveis.length];
            }
            if (variaveis[0].contains("X")) {
                for (int i = 0; i < (nColunas - variaveis.length - 1); i++) {
                    variaveisCalculo[i] = "Y" + (i + 1);
                }
            } else {
                for (int i = 0; i < (nColunas - variaveis.length - 1); i++) {
                    variaveisCalculo[i] = "X" + (i + 1);
                }
            }
        }
        for (int i = variaveis.length; i < variaveisCalculo.length; i++) {
        ...
        variaveisCalculo[variaveisCalculo.length - 1] = "B";
        
        return variaveisCalculo;
    }

    /**
     * TODO COMMENT TODO UNIT TEST
     *
    ...
    /**
     * TODO Devolve o index da linha pivot
     *
     * @param colPivot
     * @param ultimaColuna
     * @return
    ...
    /**
     * TODO UNIT TEST Divide um array pelo valor do elemento pivot
     *
     * @param indicePivot
     */
    ...
    /**
     * TODO UNIT TEST Torna todos os valores do array a zero
     *
     * @param indicesDoPivot
     * @param linha
     */
